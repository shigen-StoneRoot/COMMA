...
from batchgenerators.augmentations.spatial_transformations import augment_spatial_2, \
    ...
    augment_mirroring, augment_transpose_axes, augment_zoom, augment_resize, augment_rot90
import numpy as np
from spatial_transformations import augment_spatial
...
class SpatialTransform(AbstractTransform):
    ...
    def __init__(self, patch_size, patch_center_dist_from_border=30,
                 ...
                 label_key="seg", glb_data_key="glb_data", p_el_per_sample=1, p_scale_per_sample=1, p_rot_per_sample=1,
    ...
    def __call__(self, **data_dict):
        ...
        ret_val = augment_spatial(data, seg, patch_size=patch_size,
                                  ...
                                  p_independent_scale_per_axis=self.p_independent_scale_per_axis, glb_data=glb_data)
        data_dict[self.data_key] = ret_val[0]
        data_dict['angles'] = ret_val[-1]
        if seg is not None:
            data_dict[self.label_key] = ret_val[1]

        if glb_data is not None:
            data_dict[self.glb_data_key] = ret_val[2]
